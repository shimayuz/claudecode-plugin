...
class DeletedTextWidget extends WidgetType {
  ...
  toDOM(): HTMLElement {
    ...
    span.className = "ccd-deleted-inline";
...
class ChangeHoverWidget extends WidgetType {
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    wrap.className = "ccd-hover-actions";
        ...
        if (op === 0) {
          ...
          if (text.length > 0 && pos >= 0 && end <= docLen) {
            decos.push({
              ...
              deco: Decoration.mark({ class: "ccd-inserted-inline" }),
            });
            lastVisualPos = end;
          }
          pos = end;
